...
window.Modernizr = function(window, document, undefined) {
	...
	var featureName, hasOwnProp, version = "2.8.3", Modernizr = {}, enableClasses = !0, docElement = document.documentElement, mod = "modernizr", modElem = document.createElement(mod), mStyle = modElem.style, inputElem = document.createElement("input"), smile = ":)", toString = {}.toString, prefixes = " -webkit- -moz- -o- -ms- ".split(" "), omPrefixes = "Webkit Moz O ms", cssomPrefixes = omPrefixes.split(" "), domPrefixes = omPrefixes.toLowerCase().split(" "), ns = {svg: "http://www.w3.org/2000/svg"}, tests = {}, inputs = {}, attrs = {}, classes = [], slice = classes.slice, injectElementWithStyles = function(rule, callback, nodes, testnames) {
	...
	return Modernizr.input || webforms(), Modernizr.addTest = function(feature, test) {
		...
		return Modernizr
	}, setCss(""), modElem = inputElem = null, Modernizr._version = version, Modernizr._prefixes = prefixes, Modernizr._domPrefixes = domPrefixes, Modernizr._cssomPrefixes = cssomPrefixes, Modernizr.mq = testMediaQuery, Modernizr.hasEvent = isEventSupported, Modernizr.testProp = function(prop) {
	...
	window.espn_ui = window.espn_ui || {}, window.espn_ui.device = {}, navigator.userAgent.match(/Mobi/) ? navigator.userAgent.match(/iPad/) ? window.espn_ui.device.isTablet = !0 : window.espn_ui.device.isMobile = !0 : navigator.userAgent.match(/Tab/) ? window.espn_ui.device.isTablet = !0 : navigator.userAgent.match(/Android/) ? window.espn_ui.device.isTablet = !0 : navigator.userAgent.match(/Touch/) && navigator.userAgent.match(/IE/) && (window.espn_ui.device.isTablet = !0)
}(), function() {
	document.documentElement.className += window.espn_ui.device.isMobile ? " mobile-ua " : " desktop-ua "
}(), window.matchMedia || (window.matchMedia = function() {
	"use strict";
	var styleMedia = window.styleMedia || window.media;
	if (!styleMedia) {
		var style = document.createElement("style"), script = document.getElementsByTagName("script")[0], info = null;
		style.type = "text/css", style.id = "matchmediajs-test", script.parentNode.insertBefore(style, script), info = "getComputedStyle" in window && window.getComputedStyle(style, null) || style.currentStyle, styleMedia = {matchMedium: function(media) {
	...
	function picturefill(opt) {
		...
		for (var i = 0, plen = elements.length; plen > i; i++)
			if (element = elements[i], parent = element.parentNode, firstMatch = void 0, candidates = void 0, "IMG" === element.nodeName.toUpperCase() && (element[pf.ns] || (element[pf.ns] = {}), options.reevaluate || !element[pf.ns].evaluated)) {
				if ("PICTURE" === parent.nodeName.toUpperCase()) {
					if (pf.removeVideoShim(parent), firstMatch = pf.getMatch(element, parent), firstMatch === !1)
						continue
				} else
					firstMatch = void 0;
				("PICTURE" === parent.nodeName.toUpperCase() || element.srcset && !pf.srcsetSupported || !pf.sizesSupported && element.srcset && element.srcset.indexOf("w") > -1) && pf.dodgeSrcset(element), firstMatch ? (candidates = pf.processSourceSet(firstMatch), pf.applyBestCandidate(candidates, element)) : (candidates = pf.processSourceSet(element), (void 0 === element.srcset || element[pf.ns].srcset) && pf.applyBestCandidate(candidates, element)), element[pf.ns].evaluated = !0
			}
	}
	function runPicturefill() {
		function checkResize() {
			w._picturefillWorking || (w._picturefillWorking = !0, w.clearTimeout(resizeThrottle), resizeThrottle = w.setTimeout(function() {
				windowWidth = getWindowWidth(), picturefill({reevaluate: !0}), w._picturefillWorking = !1
			}, 250))
		}
		pf.initTypeDetects(), picturefill();
		var resizeThrottle, intervalId = setInterval(function() {
	...
	if (w.HTMLPictureElement)
		return void expose(function() {
		});
	doc.createElement("picture");
	var pf = w.picturefill || {}, getWindowWidth = function() {
		var w = window, d = document, e = d.documentElement, g = d.getElementsByTagName("body")[0];
		return parseInt(w.innerWidth || e.clientWidth || g.clientWidth, 10)
	}, windowWidth = getWindowWidth();
	pf.ns = "picturefill", function() {
		pf.srcsetSupported = "srcset" in image, pf.sizesSupported = "sizes" in image
		...
		var cssValue;
		if (!length || length.indexOf("%") > -1 != !1 || !(parseFloat(length) > 0 || length.indexOf("calc(") > -1))
			return !1;
		length = length.replace("vw", "%"), pf.lengthEl || (pf.lengthEl = doc.createElement("div"), pf.lengthEl.style.cssText = "border:0;display:block;font-size:1em;left:0;margin:0;padding:0;position:absolute;visibility:hidden", pf.lengthEl.className = "helper-from-picturefill-js"), pf.lengthEl.style.width = "0px";
		try {
			pf.lengthEl.style.width = length
		} catch (e) {
		}
		return doc.body.appendChild(pf.lengthEl), cssValue = pf.lengthEl.offsetWidth, 0 >= cssValue && (cssValue = !1), doc.body.removeChild(pf.lengthEl), cssValue
		...
		pf.types["image/jpeg"] = !0, pf.types["image/gif"] = !0, pf.types["image/png"] = !0, pf.types["image/svg+xml"] = doc.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#Image", "1.1"), pf.types["image/webp"] = pf.detectTypeSupport("image/webp", "data:image/webp;base64,UklGRh4AAABXRUJQVlA4TBEAAAAvAAAAAAfQ//73v/+BiOh/AAA=")
	}, pf.verifyTypeSupport = function(source) {
		var type = source.getAttribute("type");
		if (null === type || "" === type)
			return !0;
		var pfType = pf.types[type];
		return "string" == typeof pfType && "pending" !== pfType ? (pf.types[type] = pf.detectTypeSupport(type, pfType), "pending") : "function" == typeof pfType ? (pfType(), "pending") : pfType
		...
		for (var winningLength, sourceSizeList = pf.trim(sourceSizeListStr).split(/\s*,\s*/), i = 0, len = sourceSizeList.length; len > i; i++) {
			...
			if (length && (!media || pf.matchesMedia(media)) && (winningLength = pf.getWidthFromLength(length)))
				break
		}
		...
		for (var candidates = []; "" !== srcset; ) {
			...
			if (-1 !== pos) {
				...
				if (("," === last || "" === url) && (url = url.replace(/,+$/, ""), descriptor = ""), srcset = srcset.slice(pos + 1), null === descriptor) {
					var descpos = srcset.indexOf(",");
					-1 !== descpos ? (descriptor = srcset.slice(0, descpos), srcset = srcset.slice(descpos + 1)) : (descriptor = srcset, srcset = "")
				}
			} else
				url = srcset, srcset = "";
			(url || descriptor) && candidates.push({url: url,descriptor: descriptor})
		}
		return candidates
	}, pf.parseDescriptor = function(descriptor, sizesattr) {
		var widthInCssPixels, resCandidate, sizes = sizesattr || "100vw", sizeDescriptor = descriptor && descriptor.replace(/(^\s+|\s+$)/g, "");
		if (pf.getWidthFromLength() || -1 !== sizes.indexOf(" ") || (sizes.indexOf("px") === sizes.length - 2 ? widthInCssPixels = parseInt(sizes, 10) : sizes.indexOf("vw") === sizes.length - 2 && (widthInCssPixels = parseInt(sizes, 10) / 100 * windowWidth)), widthInCssPixels || (widthInCssPixels = pf.findWidthFromSourceSize(sizes)), sizeDescriptor)
			for (var splitDescriptor = sizeDescriptor.split(" "), i = splitDescriptor.length - 1; i >= 0; i--) {
		...
		for (var candidates = pf.parseSrcset(srcset), formattedCandidates = [], i = 0, len = candidates.length; len > i; i++) {
			...
			formattedCandidates.push({url: candidate.url,resolution: pf.parseDescriptor(candidate.descriptor, sizes)})
		}
		...
		return "IMG" === el.nodeName.toUpperCase() && el[pf.ns] && el[pf.ns].srcset && (srcset = el[pf.ns].srcset), srcset && (candidates = pf.getCandidatesFromSourceSet(srcset, sizes)), candidates
	}, pf.backfaceVisibilityFix = function(picImg) {
		var style = picImg.style || {}, WebkitBackfaceVisibility = "webkitBackfaceVisibility" in style, currentZoom = style.zoom;
		WebkitBackfaceVisibility && (style.zoom = ".999", WebkitBackfaceVisibility = picImg.offsetWidth, style.zoom = currentZoom)
		...
		return function(picImg, bestCandidate) {
			var img;
			picImg[pf.ns] && !w.pfStopIntrinsicSize && (void 0 === picImg[pf.ns].dims && (picImg[pf.ns].dims = picImg.getAttribute("width") || picImg.getAttribute("height")), picImg[pf.ns].dims || (bestCandidate.url in urlCache ? setSize(picImg, urlCache[bestCandidate.url], bestCandidate.resolution) : (img = doc.createElement("img"), img.onload = function() {
				if (urlCache[bestCandidate.url] = img.width, !urlCache[bestCandidate.url])
					try {
						doc.body.appendChild(img), urlCache[bestCandidate.url] = img.width || img.offsetWidth, doc.body.removeChild(img)
					} catch (e) {
					}
				picImg.src === bestCandidate.url && setSize(picImg, urlCache[bestCandidate.url], bestCandidate.resolution), picImg = null, img.onload = null, img = null
		...
		for (var i = 0; length > i; i++)
			if (candidate = candidates[i], candidate.resolution >= pf.getDpr()) {
				bestCandidate = candidate;
				break
			}
		bestCandidate && (bestCandidate.url = pf.makeUrl(bestCandidate.url), picImg.src !== bestCandidate.url && (pf.restrictsMixedContent() && "http:" === bestCandidate.url.substr(0, "http:".length).toLowerCase() ? void 0 !== window.console && console.warn("Blocked mixed content image " + bestCandidate.url) : (picImg.src = bestCandidate.url, picImg.currentSrc = picImg.src, pf.backfaceVisibilityFix(picImg))), pf.setIntrinsicSize(picImg, bestCandidate))
	}, pf.ascendingSort = function(a, b) {
		return a.resolution - b.resolution
	}, pf.removeVideoShim = function(picture) {
		var videos = picture.getElementsByTagName("video");
		if (videos.length) {
			for (var video = videos[0], vsources = video.getElementsByTagName("source"); vsources.length; )
...
!function(window, document) {
	function _key(k) {
		return "espn.viewcache." + k
	}
	var _canStore = Modernizr.localstorage, _cachesRendered = {};
	espn.viewCache = {get: function(key) {
		var expire, out = null, html = null;
		return _canStore && (html = window.localStorage[_key(key)], html && html.length >= 13 && (expire = parseInt(html.substring(0, 13), 10), html = isNaN(expire) ? html : html.substring(13), this.hasExpired(expire) || (out = html))), out
	},render: function(key) {
		var html, scripts = document.getElementsByTagName("script"), me = scripts && scripts.length > 0 && scripts[scripts.length - 1] || null, pnode = me && me.parentNode || null;
		return me ? (pnode && pnode.removeChild(me), html = this.get(key), null !== html ? (pnode.innerHTML = html, pnode.setAttribute("data-loadtype", "viewcache"), _cachesRendered[key] = !0, !0) : !1) : !1
	},didRenderCached: function(key) {
		return !!_cachesRendered[key]
	},hasExpired: function(expire) {
		return expire >= 0 && expire < (new Date).getTime()
	},store: function(key, html, ttl) {
		if (_canStore) {
			if (arguments.length > 2) {
				var d = new Date;
				html = "" + (d.getTime() + 1e3 * ttl) + html
			}
			try {
				window.localStorage[_key(key)] = html || ""
			} catch (e) {
			}
		}
	},storeNode: function(key, node, ttl) {
		this.store(key, node.innerHTML, ttl)
	},remove: function(key) {
		if (_canStore)
			try {
				delete window.localStorage[_key(key)]
			} catch (e) {
			}
	}}
	...
	function g(a) {
		...
		return d(c), d(g), d(h), e(h, g.length), {name: b[0],abbrs: f(b[1].split(" "), g),offsets: f(c, g),untils: h}
	}
	function h(a) {
		a && this._set(g(a))
	}
	...
	function l() {
		var a, b = [];
		for (a in y)
			y.hasOwnProperty(a) && y[a] && b.push(y[a].name);
		return b.sort()
	}
	function m(a) {
		var b, c;
	...
	function u(b) {
		var c = Array.prototype.slice.call(arguments, 0, -1), d = arguments[arguments.length - 1], e = k(d), f = a.utc.apply(null, c);
	...
	if (void 0 !== a.tz)
		return a;
	var x = "0.3.0", y = {}, z = {}, A = a.version.split("."), B = +A[0], C = +A[1];
	(2 > B || 2 === B && 6 > C) && t("Moment Timezone requires Moment.js >= 2.6.0. You are using Moment.js " + a.version + ". See momentjs.com"), h.prototype = {_set: function(a) {
...
espn.dates = function(moment, console) {
	function _moment(dateISO) {
		...
		return cached ? (_hits++, cached) : (_misses++, cached = moment(dateISO), _cache[dateISO] = cached, cached)
	}
	function _getDateET(dateISO) {
		var key = dateISO + "-" + _easternTimeZone, cached = _cache[key];
		return cached ? (_hits++, cached) : (_misses++, cached = moment.tz(dateISO, _easternTimeZone), _cache[key] = cached, cached)
	}
	...
	function _debug() {
		var total = _hits + _misses, ratio = total > 0 ? 1 * _hits / (1 * total) : 0;
		console.log("moments created: " + _misses), console.log("moments called for: " + (_misses + _hits)), console.log("cache ratio: " + ratio), console.log("cache", _cache)
	}
	var _cache = {}, _hits = 0, _misses = 0, _easternTimeZone = "America/New_York", _dateFormat = "YYYYMMDD";
	return {moment: function(dateISO) {
...
}(window.moment, window.console), function(root) {
	var jstz = function() {
		"use strict";
